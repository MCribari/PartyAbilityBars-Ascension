-- /dump GetTalentInfo(?, ?, specGroupIndex)

local lower, match, format = string.lower, string.match, string.format
local remove, wipe, unpack, pairs, ipairs = table.remove, wipe, unpack, pairs, ipairs
...
local defaultAbilities = {
	...
	["SHAMAN"] = {
		...
		[2484] = 10.5, -- Тотем оков земли 
		[60043] = 8, -- Выброс лавы 
		[16190] = 300, -- Тотем прилива маны 
	},
	["WARLOCK"] = {
		[19647] = 24,  -- Spell Lock
	...
	["Orc"] = {
		[20572] = 120, -- Кровавое неистовство
	},
	["Troll"] = {
		[26297] = 120, -- Берсерк
	},
	["NightElf"] = {
		[58984] = 120, -- Слиться с тенью
	},
	["Draenei"] = {
		[59547] = 120, -- Дар наару
	},
	...
	["Items"] = {
		...
		[71586] = 120, -- Костяной ключ
		[71638] = 60, -- Безупречный клык Синдрагосы
	},
}

local RuneOfAccuracy = "Titan-Forged Rune of Accuracy"
local RuneOfAudacity = "Titan-Forged Rune of Audacity"
local RuneOfAlacrity = "Titan-Forged Rune of Alacrity"
local RuneOfDetermination = "Titan-Forged Rune of Determination"
local RuneOfCruelty = "Titan-Forged Rune of Cruelty"

local AllianceMedallion, HordeMedallion = "Medallion of the Alliance", "Medallion of the Horde"
local BaubleTrueBlood, SkeletonKey = "Bauble of True Blood", "Corroded Skeleton Key"
local SindragosaFlawlessFang = "Sindragosa's Flawless Fang"

if GetLocale() == "ruRU" then
	RuneOfAccuracy = "Руна меткости из кузни титанов"
	RuneOfAudacity = "Руна отваги из кузни титанов"
	RuneOfAlacrity = "Руна рвения из кузни титанов"
	RuneOfDetermination = "Руна решимости из кузни титанов"
	RuneOfCruelty = "Руна жестокости из кузни титанов"

	AllianceMedallion = "Медальон Альянса" HordeMedallion = "Медальон Орды"
	BaubleTrueBlood = "Подвеска истинной крови" SkeletonKey = "Проржавевший костяной ключ"
	SindragosaFlawlessFang = "Безупречный клык Синдрагосы"
end

local itemRuneOfAccuracy = {
	[GetSpellInfo(42292)] = RuneOfAccuracy,--GetItemInfo(46083)
}
local itemRuneOfAudacity = {
	[GetSpellInfo(42292)] = RuneOfAudacity,--GetItemInfo(46081)
}
local itemRuneOfAlacrity = {
	[GetSpellInfo(42292)] = RuneOfAlacrity,--GetItemInfo(46085)
}
local itemRuneOfDetermination = {
	[GetSpellInfo(42292)] = RuneOfDetermination,--GetItemInfo(46082)
}
local itemRuneOfCruelty = {
	[GetSpellInfo(42292)] = RuneOfCruelty,--GetItemInfo(46084)
}
local itemForSpellAlliance = {
	[GetSpellInfo(42292)] = AllianceMedallion,--GetItemInfo(51377), -- PvP Trinket (id: 42124, 226lvl)
	[GetSpellInfo(71607)] = BaubleTrueBlood,--GetItemInfo(50354), -- Bauble of True Blood -- (id: 50726, 277lvl)
	[GetSpellInfo(71586)] = SkeletonKey,--GetItemInfo(50356), -- Костяной ключ
	[GetSpellInfo(71638)] = SindragosaFlawlessFang,--GetItemInfo(50361), -- Безупречный клык Синдрагосы
}
local itemForSpellHorde = {
	[GetSpellInfo(42292)] = HordeMedallion,--GetItemInfo(51378), -- PvP Trinket (id: 42126, 226lvl)
}

local iconPaths = {}

--[[for k, v in pairs(iconPaths) do
	iconPaths[GetSpellInfo(k)] = select(3,GetSpellInfo(k))
end]]

local factionIcon
if UnitFactionGroup("player") == "Alliance" then
	factionIcon = "Interface\\Icons\\Inv_jewelry_trinketpvp_01"
elseif UnitFactionGroup("player") == "Horde" then
	factionIcon = "Interface\\Icons\\Inv_jewelry_trinketpvp_02"
end

iconPaths[GetSpellInfo(42292)] = factionIcon
iconPaths[GetSpellInfo(71607)] = "Interface\\Icons\\inv_jewelcrafting_gem_28"
iconPaths[GetSpellInfo(71586)] = "Interface\\Icons\\Inv_misc_key_15"
iconPaths[GetSpellInfo(71638)] = "Interface\\Icons\\inv_jewelry_trinket_06"

-- лучше уж видеть иконку берсерка ферала чем троля
iconPaths[GetSpellInfo(50334)] = "Interface\\Icons\\Ability_Druid_Berserk"
...
local function convertspellids(t, fillwith)
	...
	for class,table in pairs(t) do
		...
		for k,v in pairs(table) do
			...
			-- CORRECCIÓN: Se añade una comprobación para evitar el error 'table index is nil' 
			-- si GetSpellInfo no devuelve un nombre válido (nil) para un Spell ID.
			if spellName then
...
local function CreateIcon(anchor)
	...
	local borderFrame = CreateFrame("Frame",anchor:GetName().."borderFrame".. (#anchor.icons+1),PABIcons)
	borderFrame:SetFrameLevel(cd:GetFrameLevel() + 1)
	borderFrame:SetHeight(30)
	borderFrame:SetWidth(30)
	...
	icon.UpdateBorder = function()
		if borderFrame.border:GetAlpha() == 0 then return end
		borderFrame.border:SetAlpha(0)
	end

	border:SetAllPoints(true)
	-- border:SetTexture("Interface\\AddOns\\PAB\\icon_border.blp")
	border:SetTexture("Interface\\AddOns\\PAB\\square_border_5px.tga")
	border:SetVertexColor(0.4549, 0, 1)
...
function PAB:AppendIcon(icons,anchor)
	local newicon, borderFrame = CreateIcon(anchor)
	iconlist[#iconlist+1] = newicon
	if #icons == 0 then
		newicon:SetPoint("TOPLEFT",anchor,"BOTTOMLEFT")
		borderFrame:SetPoint("TOPLEFT",anchor,"BOTTOMLEFT")
	elseif db.iconsperline ~= 0 and (#icons % db.iconsperline) == 0 then
		...
		borderFrame:SetPoint("TOPLEFT",icons[#icons - db.iconsperline + 1],"BOTTOMLEFT", 0, -1)
	else
		newicon:SetPoint("LEFT",icons[#icons],"RIGHT", 1, 0)
		borderFrame:SetPoint("LEFT",icons[#icons],"RIGHT", 1, 0)
	end
...
-- Utils
local function array_contains(tab, itemName, hordeTrinket, Accuracy, Audacity, Alacrity, Determination, Cruelty)
	-- V: no value given
	if not tab then return false end

    for index, value in pairs(tab) do
        if value == itemName or value == hordeTrinket
		or value == Accuracy or value == Audacity or value == Alacrity or value == Determination or value == Cruelty then
            return true
        end
    end
	...
	for i=1,GetNumPartyMembers() do
		...
		if updateIcons then
			for i = 1, #anchor.icons do
				anchor.icons[i]:Hide()
				anchor.icons[i]:SetParent(UIParent)
		...
		if anchor.items then
			for ability,cooldown in pairs(db.abilities["Items"]) do
				local itemName = itemForSpellAlliance[ability]
				local hordeTrinket = itemForSpellHorde[ability]
				local Accuracy = itemRuneOfAccuracy[ability]
				local Audacity = itemRuneOfAudacity[ability]
				local Alacrity = itemRuneOfAlacrity[ability]
				local Determination = itemRuneOfDetermination[ability]
				local Cruelty = itemRuneOfCruelty[ability]
				local enabled = db.PABenabledCooldowns["Items"][ability]
				if enabled and array_contains(anchor.items, itemName, hordeTrinket,
											  Accuracy, Audacity, Alacrity, Determination, Cruelty) then
					self:UpdateAnchorIcon(anchor, numIcons, ability, cooldown)
					numIcons = numIcons + 1
				end
			end
		end
		-- CORRECCIÓN: Se añade 'or {}' para manejar casos donde no hay habilidades definidas para la raza, 
		-- o donde 'db.PABenabledCooldowns[anchor.race]' es nil (previene 'bad argument #1 to pairs' y 'table index is nil').
		for ability,cooldown in pairs(db.abilities[anchor.race] or {}) do
			local enabled = (db.PABenabledCooldowns[anchor.race] or {})[ability]
			if enabled then
				self:UpdateAnchorIcon(anchor, numIcons, ability, cooldown)
				numIcons = numIcons + 1
			end
		...
		for ability,cooldown in pairs(db.abilities[class] or {}) do
			-- if it's not a talent, or we have the talent
			local enabled = (db.PABenabledCooldowns[class] or {})[ability]
...
-- дурак, сразу не заметил groupedCooldowns ;(
--[[local ShieldBash = select(1,(GetSpellInfo(72))) -- Удар щитом
local Pummel = select(1,(GetSpellInfo(6552))) -- Зуботычина
local enabled1 = true
local OnUpdateFrame = CreateFrame("Frame")
local Update = 0.1

function OnUpdateFrame.OnUpdate(Frame)
...
function PAB:CheckAbility(anchor,ability,cooldown,pIndex)
	...
	for k, icon in ipairs(anchor.icons) do
		...
		--[[if ability == ShieldBash and enabled1 then
			enabled1 = false
			self:CheckAbility(anchors[pIndex], Pummel, cooldown, pIndex)
			OnUpdateFrame:OnUpdate()
		elseif ability == Pummel and enabled1 then
			enabled1 = false
			self:CheckAbility(anchors[pIndex], ShieldBash, cooldown, pIndex)
			OnUpdateFrame:OnUpdate()
		end]]
	...
	if time > 0.05 then
		...
		for k,icon in ipairs(iconlist) do
			if icon.active then
				...
				if icon.cooldown - 0.8 >= icon.timeleft then
					icon.UpdateBorder()
				end
...
function PAB:QuerySpecInfo(elapsed)
	PABinspectData.throttle = PABinspectData.throttle + elapsed

	-- timeout check
	if PABinspectData.lastQuery then
		PABinspectData.lastQuery = PABinspectData.lastQuery + elapsed
		if PABinspectData.lastQuery > 10 then -- we've been waiting for 10s... give up
			PABinspectData.current = nil
			PABinspectData.lastQuery = nil
		end
	end

	-- throttle
	if PABinspectData.throttle < 0.5 then return end
	PABinspectData.throttle = 0

	-- are we fighting
	if InCombatLockdown() then return end
	-- if we're currently awaiting an INSPECT_TALENT_READY
	if PABinspectData.current then return end
	-- are we dead
	if UnitIsDead("player") then return end
	-- is the player inspecting
	if InspectFrame and InspectFrame:IsShown() then return end

	if not PABinspectData.frame then
		PABinspectData.frame = CreateFrame("Frame")
		PABinspectData.frame:SetScript("OnEvent", function (self, event, ...)
			-- are we fighting
			if InCombatLockdown() then return end
			-- is the player inspecting
			if InspectFrame and InspectFrame:IsShown() then return end

			if not PABinspectData.current then return end
			local anchor = anchors[PABinspectData.current]
			...
			anchor.items = {}
			for itemSlot = 13, 14 do
				local itemID = GetInventoryItemLink("party"..PABinspectData.current, itemSlot)
				if itemID then
		...
		PABinspectData.frame:RegisterEvent("INSPECT_TALENT_READY")
	end
	...
	for i=1, GetNumPartyMembers() do
		...
		if ( CanInspect("party"..i) and UnitIsConnected("party"..i) ) then

			if ( not anchor.spec or not anchor.items ) then

				PABinspectData.current = i
				PABinspectData.lastQuery = 0
...
local function PAB_OnLoad(self)
	...
	PABDB = PABDB or {
		...
		PABenabledCooldowns = allCooldownIds
	}
	PABDB.abilities = defaultAbilities -- V: always keep ALL the abilities
	
	-- V: upgrade path #1
	if not PABDB.PABenabledCooldowns["Items"] then
		PABDB.PABenabledCooldowns["Items"] = PABenabledCooldowns["Items"]
	end
...
function PAB:CreateOptions()
	...
	local scale = panel:MakeSlider(
		...
		'currentTextFunc', function(value) return format("%.2f",value) end)
	scale:SetPoint("TOP",subText,"TOP", 117, -190)
	self.scale = scale
	...
	arena:SetPoint("TOP",panel,"TOP", 50, -13)
	...
	local iconsperline = panel:MakeSlider(
		...
		'currentTextFunc', function(value) return format("%d",value) end)
...
function PAB:UpdateScrollBar()
	...
	for ability,cooldown in pairs(classSelectedTable) do
		if i > offset and (i - offset) <= SHOW_BUTTONS then
			...
			if not db.PABenabledCooldowns[db.classSelected] then
				db.PABenabledCooldowns[db.classSelected] = {}
			end

			local checked = db.PABenabledCooldowns[db.classSelected][ability]
			btns[i - offset]:SetChecked(checked)

			local text = GetToggleText(btns[i - offset])
			self.abilityStore[i - offset] = ability
			text:SetText(itemForSpellAlliance[ability] or ability)
		end
		i = i + 1
	end
...
function PAB:CreateAbilityEditor()
	...
	local function setterFunc(i)
		return function (value)
			...
			db.PABenabledCooldowns[db.classSelected][ability] = value
			-- redraw *everything*
			PAB:UpdateAnchors(true)
		end
	end
	...
	scrollframe:SetWidth(212);
	scrollframe:SetHeight(381)
	scrollframe:SetPoint('TOPLEFT', 5, -42)
	...
	dropdown:SetPoint("TOP",self.scale,"TOP", -20, -55)
end
